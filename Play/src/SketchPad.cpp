...
void SketchPad::onAltered()
{
	qDebug()  << "SketchPad::onAltered";
...
void SketchPad::updatePad()
{
	...
	if (_graphics2D->reset()) {
		...
		if (_backgroundImageName) {
			...
			qDebug()  << "SketchPad::updatePad: clearRect " << _backgroundColor.red << " " << _backgroundColor.green  << " " << _backgroundColor.blue << " " << _backgroundColor.alpha;
		}
...
void SketchPad::onMultitouch(MultitouchEvent *event) {
	...
	if (event->isDown()) {
		...
		if (tool() == "freedraw") {
			...
			memcpy((void*)&_strokeColors[_strokes-1], (void*)&_currentColor, sizeof(Color));
...
{
	qDebug()  << "SketchPad::saveImage" << filename << " " << _saving;
